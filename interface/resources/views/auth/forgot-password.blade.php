          ...
          <div
            ...
            <div class="w-full max-w-xs">
              <form method="POST" action="{{ route('login') }}"
                ...
                @if (session('status'))
                  <div
                    class="text-md text-green-600 text-center border-2 border-green-500 bg-green-100 rounded-md my-6">
                    {{ session('status') }}
                  </div>
                @endif
                <form method="POST" action="{{ route('password.email') }}">
                  ...
                  <div class="block">
                    <input id="email" type="email" name="email"
                           ...
                           placeholder="Give me your email &#128530;" required autofocus/>
                  </div>
                  <div class="flex items-center justify-end mt-4">
                    <button
                      class="w-full bg-red-700 hover:bg-red-800 m-auto mb-6 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                      {{ __('Reset Password') }}
                    </button>
                  </div>
                </form>
